...
This library bundles the [lwmqtt](https://github.com/256dpi/lwmqtt) MQTT 3.1.1 mqttClient and adds a thin wrapper to get an Arduino like API.
...
- The maximum size for packets being published and received is set by default to 128 bytes. To change the buffer sizes, you need to use `MQTTClient mqttClient(256)` instead of just `MQTTClient mqttClient` on the top of your sketch. The passed value denotes the read and write buffer size.

- On the ESP8266 it has been reported that an additional `delay(10);` after `mqttClient.loop();` fixes many stability issues with WiFi connections.

- To use the library with shiftr.io, you need to provide the token key (username) and token secret (password) as the second and third argument to `mqttClient.connect(name, key, secret)`. 
...
- In case you need a reference to an object that manages the mqttClient, use the `void * ref` property on the mqttClient to store a pointer, and access it directly from the advanced callback.
...
Connect to broker using the supplied mqttClient id and an optional username and password:
...
- If the `skip` option is set to true, the mqttClient will skip the network level connection and jump to the MQTT level connection. This option can be used in order to establish and verify TLS connections manually before giving control to the MQTT mqttClient. 
