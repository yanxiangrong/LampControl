...
static lwmqtt_err_t lwmqtt_drain_network(lwmqtt_client_t *client, size_t amount) {
    ...
    while (amount > 0) {
        ...
        size_t partial_read = 0;
        lwmqtt_err_t err =
                client->network_read(client->network, client->read_buf, max_read, &partial_read,
...
static lwmqtt_err_t lwmqtt_cycle(lwmqtt_client_t *client, size_t *read, lwmqtt_packet_type_t *packet_type) {
    ...
    switch (*packet_type) {
        ...
        case LWMQTT_PUBREC_PACKET: {
            ...
            size_t len;
            err = lwmqtt_encode_ack(client->write_buf, client->write_buf_size, &len, LWMQTT_PUBREL_PACKET, 0,
                                    packet_id);
            if (err != LWMQTT_SUCCESS) {
                return err;
            }
        ...
        case LWMQTT_PUBREL_PACKET: {
            ...
            size_t len;
            err = lwmqtt_encode_ack(client->write_buf, client->write_buf_size, &len, LWMQTT_PUBCOMP_PACKET, 0,
                                    packet_id);
            if (err != LWMQTT_SUCCESS) {
                return err;
            }
