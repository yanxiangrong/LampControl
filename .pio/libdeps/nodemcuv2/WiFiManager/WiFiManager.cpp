...
bool WiFiManager::addParameter(WiFiManagerParameter *p) {
    if (_paramsCount + 1 > _max_params) {
        ...
        WiFiManagerParameter **new_params = (WiFiManagerParameter **) realloc(_params, _max_params *
                                                                                       sizeof(WiFiManagerParameter *));
        if (new_params != NULL) {
            _params = new_params;
        } else {
...
void WiFiManager::setupConfigPortal() {
    ...
    server->on(String(F("/fwlink")).c_str(), std::bind(&WiFiManager::handleRoot,
